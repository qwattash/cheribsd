...
systrace_args(int sysnum, void *params, uintptr_t *uarg, int *n_args)
{
	...
	case 120: {
		...
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intptr_t)p->iovp; /* const struct iovec64 * */
	...
	case 315: {
		...
		uarg[a++] = (intptr_t)p->aiocbp; /* const struct aiocb64 * const * */
		iarg[a++] = p->nent; /* int */
	...
	case 338: {
		...
		uarg[a++] = (intptr_t)p->jail; /* struct jail64 * */
		*n_args = 1;
		break;
	}
	...
	case 345: {
		...
		uarg[a++] = (intptr_t)p->set; /* const sigset_t * */
		uarg[a++] = (intptr_t)p->info; /* struct __siginfo64 * */
		uarg[a++] = (intptr_t)p->timeout; /* const struct timespec * */
		*n_args = 3;
		break;
	}
	...
	/* freebsd64_kcmp */
	case 588: {
		struct freebsd64_kcmp_args *p = params;
		iarg[a++] = p->pid1; /* pid_t */
		iarg[a++] = p->pid2; /* pid_t */
		iarg[a++] = p->type; /* int */
	...
	/* freebsd64_setcred */
	case 591: {
		struct freebsd64_setcred_args *p = params;
		uarg[a++] = p->flags; /* u_int */
		uarg[a++] = (intptr_t)p->wcred; /* const struct setcred64 * */
		uarg[a++] = p->size; /* size_t */
		*n_args = 3;
		break;
	}
	...
	/* freebsd64_inotify_add_watch_at */
	case 593: {
		struct freebsd64_inotify_add_watch_at_args *p = params;
		iarg[a++] = p->fd; /* int */
		iarg[a++] = p->dfd; /* int */
		uarg[a++] = (intptr_t)p->path; /* const char * */
	...
	/* freebsd64_pdrfork */
	case 600: {
		struct freebsd64_pdrfork_args *p = params;
		uarg[a++] = (intptr_t)p->fdp; /* int * */
	...
	/* freebsd64_pdwait */
	case 601: {
		struct freebsd64_pdwait_args *p = params;
		iarg[a++] = p->fd; /* int */
		uarg[a++] = (intptr_t)p->status; /* int * */
		iarg[a++] = p->options; /* int */
		uarg[a++] = (intptr_t)p->wrusage; /* struct __wrusage * */
...
systrace_entry_setargdesc(int sysnum, int ndx, char *desc, size_t descsz)
{
	...
	case 120:
		...
		case 0:
			...
			break;
		case 1:
			p = "userland const struct iovec64 *";
			break;
		case 2:
			p = "u_int";
			break;
	...
	case 315:
		switch (ndx) {
		case 0:
			p = "userland const struct aiocb64 * const *";
			break;
		case 1:
			p = "int";
			break;
	...
	case 591:
		...
		case 0:
			...
			break;
		case 1:
			p = "userland const struct setcred64 *";
			break;
		case 2:
			p = "size_t";
			break;
