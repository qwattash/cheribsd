...
' < $1 | awk "
	...
	NR == 1 {
		...
		printf " * created from%s\n */\n\n", $0 > sysargmaphdr
		printf "#ifndef %s\n", sysargmap_h > sysargmaphdr
		printf "#define\t%s\n\n", sysargmap_h > sysargmaphdr
		printf "#include <sys/types.h>\n" > sysargmaphdr
		printf "#include <compat/cheriabi/%s>\n\n", syshdr > sysargmaphdr
		printf "struct %ssam {\n", syscallprefix > sysargmaphdr
		printf "\tu_char sam_return_ptr;\n" > sysargmaphdr
		printf "\tu_char sam_ptrmask;\n};\n\n", syscallprefix > sysargmaphdr
		printf("extern struct %ssam %sargmap[%sMAXSYSCALL];\n",
		       syscallprefix, syscallprefix, syscallprefix) > sysargmaphdr
		printf "#endif /* !%s */\n", sysargmap_h > sysargmaphdr

		printf " * created from%s\n */\n\n", $0 > sysargmap
		printf "#include <compat/cheriabi/%s>\n\n", sysargmaphdr > sysargmap
		printf("struct %ssam %sargmap[%sMAXSYSCALL] = {\n",
		       syscallprefix, syscallprefix, syscallprefix) > sysargmap
